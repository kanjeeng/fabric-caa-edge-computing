...
async function main() {
    console.log("🔥 Memulai LOAD GENERATOR - Skenario A (IMBALANCE 90% ke Zone-A)...");
    ...
    try {
        ...
        while (true) {
            // LOGIKA IMBALANCE (90% Zone A, 5% Zone B, 5% Zone C)
            const rand = Math.random();
            let targetZone = 'zone-a';
            if (rand > 0.90 && rand <= 0.95) targetZone = 'zone-b';
            else if (rand > 0.95) targetZone = 'zone-c';

            const spotId = `spot-imb-${counter % 1000}`; 

            try {
                // Eksekusi transaksi asynchronous agar tidak menunggu
            ...
            // FASE TIDUR SANGAT SINGKAT (50ms) untuk membuat antrian menumpuk
